/*
 * (c) 2002, 2004, 2006 Anders Torger
 *
...
delay::delay()
    : subdelay_filter(NULL)
{
}

void
delay::update(delaybuffer_t *db,
             void *buf,
             int sample_size,
             int sample_spacing,
             int delay,
                      ...
                      int sample_size)
{
    ...
    db = (delaybuffer_t *) malloc(sizeof(delaybuffer_t));
    ...
    if (delay <= fragment_size)
    {
        ...
        db->shortbuf[0] = _aligned_malloc(size, ALIGNMENT);
        db->shortbuf[1] = _aligned_malloc(size, ALIGNMENT);
    ...
    db->fbufs = (void **) malloc(db->n_fbufs_cap * sizeof(void *));
    size = fragment_size * sample_size;

    for (n = 0; n < db->n_fbufs_cap; n++)
    {
        db->fbufs[n] = _aligned_malloc(size, ALIGNMENT);
        memset(db->fbufs[n], 0, size);
    }

    if (maxdelay > 0)
    {
        db->rbuf = _aligned_malloc(size, ALIGNMENT);
...
int
delay::subsample_filterblocksize(void)
{
    return subdelay_filterblock_size;
}

void
delay::subsample_update(fftw_convolver *convolver,
                        void *buf,
                        void *rest,
                        int subdelay)
{
    ...
    for (i = 0; i < subdelay_fragment_size * realsize; i += blocksize)
    {
        ...
        convolver->convolver_td_convolve(subdelay_filter[subdelay], cbuffer);
                      ...
                      int _realsize)
{
    ...
    subdelay_filterblock_size =
        convolver->convolver_td_block_length(subdelay_filter_length);
    
    if (step_count < 2)
    {
        pinfo("Invalid step_count %d.\n", step_count);
        return false;
    }
    
    if (half_filter_length < 1)
    {
        pinfo("Invalid half filter length %d.\n", half_filter_length);
        return false;
    }

    if (fragment_size % subdelay_filterblock_size != 0)
    {
        pinfo("Incompatible fragment/filter sizes (%d/%d).\n",
    ...
    subdelay_filter = (td_conv_t **) malloc((2 * step_count + 1) * sizeof(td_conv_t *));
    subdelay_filter = &subdelay_filter[step_count];
    filter = malloc(subdelay_filter_length * realsize);
    ...
    subdelay_filter[0] = convolver->convolver_td_new(filter, subdelay_filter_length);
    free(filter);
    ...
    for (n = 1; n < step_count; n++)
    {
        filter = sample_sinc(subdelay_filter_length >> 1,
                             (double)n / step_count,
                             kaiser_beta);
        subdelay_filter[n] = convolver->convolver_td_new(filter, subdelay_filter_length);
...
void *
delay::sample_sinc(int half_length,
                   double offset,
                   double kaiser_beta)
{
    int n, filter_length;
    ...
    filter = _aligned_malloc(filter_length * realsize, ALIGNMENT);
...
void
delay::copy_to_delaybuf(void *dbuf,
                        void *buf,
                        int sample_size,
                        int sample_spacing,
                        int n_samples)
{
    ...
    case 8:
        ...
        break;
    default:
        pinfo("Sample byte size %d not suppported.\n", sample_size);
        break;
    }
}

void
delay::copy_from_delaybuf(void *buf,
                          void *dbuf,
                          int sample_size,
                          int sample_spacing,
                          int n_samples)
...
void
delay::update_delay_buffer(delaybuffer_t *db,
                           int sample_size,
                           int sample_spacing,
                           uint8_t *buf)
{
    ...
    lastbuf = (uint8_t *)((db->curbuf == db->n_fbufs - 1) 
                              ? db->fbufs[0] 
...
void
delay::update_delay_short_buffer(delaybuffer_t *db,
                                 int sample_size,
                                 int sample_spacing,
                                 uint8_t *buf)
{
...
void
delay::change_delay(delaybuffer_t *db,
                    int sample_size,
                    int newdelay)
{
    int size, i;
